...
$(document).ready(function(){
	$("#refreshAttend").on("click",function () {
		...
		console.log($("#course option[value="+course_name+"]"));
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i = 0;i<data.length;i++){
				InitOption(pnode,data[i].teacher_id,data[i].teacher_name);
			}
			$("#teacher").on("click",function () {
				var teacher_name = $(this).val();
				console.log($("#course option[value='"+teacher_name+"']"));
				var teacher_id =  $("#teacher option[value='"+teacher_name+"']").attr('data-id');
				var data_info = {"teacher_id":teacher_id};
				console.log(data_info);
				RequestCourse(data_info);
			})
		}
	});
}
...
/************获得课程普通教师******************/
function RequestCourse(data_info){
	$.ajax({
		type:"post",
		url:"/adminGetMyTeach",
		async:true,
		...
		success:function(data){
			...
			$("#course").empty();
			for(var i = 0;i<data.length;i++){
				InitOption(pnode,data[i].course_id,data[i].course_name);
			}
			$("#course").on("click",function () {
				var course_name = $(this).val();
				console.log(course_name);
				var course_id =  $("#course option[value="+course_name+"]").attr('data-id');
				var mydata_info = {"course_id":course_id,"teacher_id":data_info.teacher_id};
				console.log(mydata_info);
				Requestclass(mydata_info);
			})
		}
	});
}
